...
function renderScenario(state) {
  ...
  tabsEl.innerHTML = "";
  if (currentScenarioTabIndex < 0 || currentScenarioTabIndex >= scenario.sections.length) currentScenarioTabIndex = 0;
  scenario.sections.forEach((section, idx) => {
    ...
    // scenario 화면 자체가 다시 렌더될 때마다 핸들러를 직접 부착한다.
    // 상위 이벤트 바인딩이 중간에 실패해도 탭은 독립적으로 작동한다.
    btn.onclick = () => {
      currentScenarioTabIndex = idx;
      renderScenario(latestState || state);
    };
  ...
  const readyBtn = el("scenario-ready-btn");
  readyBtn.disabled = already;
  readyBtn.textContent = already ? "준비완료 (대기 중)" : "준비완료";
  // 준비완료 역시 scenario 렌더러가 직접 책임진다. main.js의 정적 이벤트
  // 연결 여부와 무관하게 Firebase markReady가 실행되도록 한다.
  readyBtn.onclick = async () => {
    if (isLocalGM() || !latestState || readyBtn.disabled) return;
    try {
      readyBtn.disabled = true;
      await markReady("scenario-" + latestState.currentPhase);
    } catch (err) {
      readyBtn.disabled = false;
      console.error("개인 시나리오 준비완료 오류:", err);
      alert("준비 완료 처리 중 오류가 발생했습니다. 새로고침 후 다시 눌러주세요.");
    }
  };
}
