...
function renderRules(state) {
  ...
  const ruleText = RULES[ruleKey] || "";
  if (["basic", "phase1", "phase1Progress", "phase2", "phase3"].includes(ruleKey)) {
    ...
    const sourceLines = ruleText.split("\n");
    const rendered = [];
    ...
    sourceLines.forEach((line, lineIndex) => {
      ...
      if (ruleKey === "basic" && rendered.length === 0 && t === "개인 시나리오") rendered.push('<div class="rule-first-spacer" aria-hidden="true"></div>');
      const isProgressSection = (ruleKey === "phase2" && t === "<2페이즈 진행>") || (ruleKey === "phase3" && t === "<3페이즈 진행>");
      if (isProgressSection) {
        rendered.push('<div class="rule-section-divider" aria-hidden="true">── ✦ ──</div>');
      } else if (spacerPending && !previousWasHeading) {
        rendered.push('<div class="rule-spacer" aria-hidden="true"></div>');
      }
      spacerPending = false;
      if (isHeading) rendered.push(`<div class="rule-line rule-heading${((ruleKey === 'phase2' && (t === '<2페이즈 준비>' || t === '<2페이즈 진행>')) || (ruleKey === 'phase3' && (t === '<3페이즈 준비>' || t === '<3페이즈 진행>'))) ? ' rule-section-heading' : ''}">${safeLine}</div>`);
      else if (warnings.has(t)) rendered.push(`<div class="rule-line rule-warning">${safeLine}</div>`);
      else rendered.push(`<div class="rule-line">${safeLine}</div>`);
      previousWasHeading = isHeading;
    });
    el("rules-text").innerHTML = rendered.join("");
  } else {
    el("rules-text").textContent = ruleText;
  }
...
function updateScenarioReviewButton(state) {
  ...
  if (!btn) return;
  const hasCharacter = !!getLocalCharacterId() && !isLocalGM();
  ...
  QUADRANT_CHARACTER_IDS.forEach((characterId, idx) => {
    ...
    if (maxPhase >= 2) {
      ...
      for (let phase = 1; phase <= maxPhase; phase++) {
        ...
        tab.addEventListener("click", () => { quadrantPhaseView[character.id] = phase; render(); });
...
function openClaimedClueModal(clueId) {
  ...
  const canReveal = myCharId && myCharId === ownerCharId && !alreadyRevealed;
...
function renderInvestigation(state) {
  ...
  el("investigation-title").textContent = `${round}차 조사`;

  const sequence = getRoundSequence(state.currentPhase, round);
  const turnIndex = state.investigationTurnIndex || 0;
  const currentTurnCharId = sequence[turnIndex];
  const currentTurnChar = findCharacter(currentTurnCharId);
  const myCharId = getLocalCharacterId();

  el("investigation-turn-indicator").textContent = "";
  ...
  if (!roomUnlocked) {
    ...
    btn.disabled = claimed || !isMyTurn || isOwnRoom;
...
function renderInvestigationFree(state) {
  showOnlyStage(el("stage-investigation"));
  ...
  el("investigation-title").textContent = "조사";

  const myCharId = getLocalCharacterId();
  const myCount = getFreePickCount(state, myCharId);
  el("investigation-turn-indicator").textContent = `자유롭게 원하는 단서를 선택하세요. (나: ${myCount}/${picksPerPerson} 획득)`;
  el("investigation-turn-indicator").className = "scenario-text";
  ...
  if (!room) {
    ...
    if (roomLocked) {
      ...
      room.items.forEach((item, idx) => {
        ...
        btn.textContent = letter;
        btn.dataset.clueId = item.id;
        btn.disabled = claimed || reachedLimit;
        letterRow.appendChild(btn);
      });
...
function renderArrest(state) {
  ...
  if (revoteCount > 0 && eligibleIds) {
    const names = eligibleIds
      .map((id) => (config.targets ? config.targets.find((t) => t.id === id)?.name : null) || id)
      .join(", ");
