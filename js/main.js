...
async function onStageTimeUp(state) {
  const startedAt = state.stageStartedAt;

  if (state.currentStage === "scenario") {
    const after = AFTER_SCENARIO[state.currentPhase];
    if (after) await transitionIfStillOn("scenario", after.stage, after.durationSec, startedAt, true);
    return;
  }
  ...
  if (talkMatch) {
    ...
    // 3차 밀담 뒤 전체 토론에는 토론 시간(20분)을 걸어준다. (예전엔 0이라 타이머가 없었음)
    const nextDuration = nextStage === "discussion" ? DISCUSSION_STAGES["discussion"].durationSec : 0;
    await transitionIfStillOn(state.currentStage, nextStage, nextDuration, startedAt, true);
    return;
  }
  ...
  if (!after) return;
  await transitionIfStillOn(state.currentStage, after.stage, after.durationSec, startedAt, true);
}
